<cfcomponent displayname="status" hint="User Out-Of-Office logic">

	<cfset variables.dao = APPLICATION.availability.objectFactory.getObjectByName("OutOfOffice", "dao","availability")>
	...
	<cffunction name="get" access="remote" returntype="array" output="true">
		...
		<cfquery name="local.q" datasource="#variables.config.getDSN()#">
			...
			<cfif structkeyexists(arguments.data, "id")>
				where o.tartanid = 
				<cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.data.id#">
			</cfif>order by o.starttime desc
		</cfquery>
	...
	<cffunction name="save" access="remote" returntype="any" output="true">
		...
		<!--- <cfmail to="brian.cooney@sinclair.edu" from="availability@sinclair.edu" subject="wtf" 
		type="html">
