component output="false" hint="OutOfOffice_DAO"
{
	// The name of the relevant entity/bean for this dao
	variables.entityName = "OutOfOffice";
	...
	public array function selectPaged(required numeric startIndex, required numeric numItems, 
	                                  string sortOrder,struct filter=StructNew())
	{
		return entityload(variables.entityName, arguments.filter, arguments.sortOrder, 
	                   {offset=arguments.startIndex, maxresults=arguments.numItems});
	}
	...
	{
		return ormexecutequery("select count(*) from OutOfOffice", true);
	}
