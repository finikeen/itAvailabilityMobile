Ext.define('Adm.util.FormRendererUtils',{	
    ...
    getProfileFormItems: function(){
		var cleaner = Ext.create( 'Adm.util.TemplateDataUtil' );
	...
	{
		return new Adm.model.FieldError({
			label: field.fieldLabel.replace(Adm.util.Constants.REQUIRED_ASTERISK_DISPLAY,"","gi"),
	...
	displayErrors: function( fields ){
		...
		Ext.create('Adm.view.ErrorWindow', {
		    height: 300,
		    width: 500
		}).show();
	},
	...
	displaySaveSuccessMessage: function( scope ){
		...
		var id = setTimeout(hideMessage, Adm.util.Constants.DATA_SAVE_SUCCESS_MESSAGE_TIMEOUT);
		me.show();
	},
