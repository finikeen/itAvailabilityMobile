Ext.define('Adm.model.AuthenticatedPerson', {
    ...
    config: {
		fields: [{
			...
			name: 'objectPermissionsCollection',
			type: 'auto'
		}]
	},  
    statics: {
    ...
    hasAccess: function( objectName ){
		...
		*/
		...
		// TEMPORARILY RETURN ALL ACCESS AS TRUE, UNTIL SECURITY IS IMPLEMENTED
		return true;
    },
    ...
    setObjectPermissions: function(){
    	...
    	for (prop in Adm.model.AuthenticatedPerson)
    	{
    		if( prop.search(re) != -1)
    		{
    			requiredPermissions = Adm.model.AuthenticatedPerson[prop];
    			hasAccess = me.hasRequiredPermissions(requiredPermissions);
    			objectPermission = new Adm.model.ObjectPermission;
    ...
    isDeveloperRestrictedContent: function( item ){
		...
		if (item instanceof Adm.model.reference.ConfidentialityLevel)
		{
			restricted = true;
		}
		return restricted;
